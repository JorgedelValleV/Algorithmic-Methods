...
public:
   ...
   void kesimo(std::vector<int>const & vec) {
	   for (int i = 0; i < vec.size(); ++i) {
		   if (vec[i] > this->size())
			   std::cout << "??" << '\n';
		   else std::cout << elementok(this->raiz, vec[i]) << '\n';
	   }
   }
   ...
   int elementok(Link r, int k) {
	   if (r == nullptr)
		   ...
		   if (r->tam_i == k)
			   ...
			   if (r->tam_i < k)
				   return elementok(r->dr, k - r->tam_i);
			   else
				   return elementok(r->iz, k);
		   }
	   }
   }

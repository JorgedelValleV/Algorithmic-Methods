...
public:
   ...
   void rango(int k1, int k2) {
	   return rango(this->raiz, k1, k2);
   }
   
protected:
	void rango(Link r, int min, int max) {
		if (r==nullptr) {}
		else {
			if (r->cv.clave <= min) {
				if (r->cv.clave == min) {
			...
			else {	//la raiz es mayor al mínimo

				if (r->cv.clave <= max) {//Puedo investigar a su izquierda y derecha, porque la riaz esta en el intervalo [min,max]
					rango(r->iz, min, max);
					std::cout << r->cv.clave << ' ';
					if (r->cv.clave < max)// Todavia puede haber elementos entre la raíz y el maximo
						rango(r->dr, min, max);
				}
